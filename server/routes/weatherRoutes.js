...
// -----------------------------
router.post('/current', authMiddleware, async (req, res) => {
    try {
        const { city, lat, lon } = req.body;
        let url;
        ...
        // ✅ SAVE SEARCH HISTORY (only if city exists)
        if (city) {
...
router.post('/forecast', authMiddleware, async (req, res) => {
    try {
        ...
        res.json({
            success: true,
            city: response.data.city,
            forecast: forecastData.slice(0, 8) // next 24 hours
        });
...
// 🌫️ Air Quality
// -----------------------------
router.post('/air-quality', authMiddleware, async (req, res) => {
    try {
        const { lat, lon } = req.body;
